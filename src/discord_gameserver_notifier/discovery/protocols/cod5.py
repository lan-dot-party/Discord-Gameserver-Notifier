"""
Call of Duty 5: World at War protocol implementation for game server discovery.
"""
...
class CoD5Protocol(ProtocolBase):
    """Call of Duty 5: World at War protocol handler for broadcast discovery"""
    
    def __init__(self, timeout: float = 5.0):
        # CoD5 uses port 28960 for both source and destination
        super().__init__("255.255.255.255", 28960, timeout)
        self._allow_broadcast = True
        self.timeout = timeout
        self.logger = logging.getLogger(__name__)
        
        # CoD5 broadcast query configuration
        self.protocol_config = {
            'port': 28960,  # CoD5 standard port
    ...
    def get_discord_fields(self, server_info: dict) -> list:
        """
        Get Discord embed fields for CoD5 server information.
        
        Args:
            server_info: Dictionary containing CoD5 server information
        ...
        # PunkBuster - ALWAYS show (CoD5 specific)
        pb_status = additional_info.get('pb', '0') == '1'
        fields.append({
            'name': '🔒 PunkBuster',
            'value': '✅ Aktiviert' if pb_status else '❌ Deaktiviert',
            'inline': True
        })
        ...
        # Hardware info - ALWAYS show (CoD5 specific)
        hw_info = additional_info.get('hw', '0')
        hw_status = 'Unbekannt' if hw_info == '0' else f'Hardware: {hw_info}'
        fields.append({
            'name': '💻 Hardware',
            'value': hw_status,
            'inline': True
    ...
    def _translate_gametype(self, gametype_code: str) -> str:
        ...
        gametype_translations = {
            ...
            'twar': 'War (Capture the Flag)'  # CoD5 specific
        }
    ...
    async def scan_servers(self, scan_ranges: List[str]) -> List[ServerResponse]:
        """
        Scan for CoD5 servers using broadcast discovery.
        
        Args:
            scan_ranges: List of network ranges to scan
            
        Returns:
            List of ServerResponse objects for CoD5 servers
        """
        servers = []
        port = self.protocol_config['port']
        
        self.logger.debug("Starting CoD5 broadcast discovery")
        ...
        try:
            ...
            for scan_range in scan_ranges:
                try:
                    ...
                    self.logger.debug(f"Broadcasting CoD5 discovery to {broadcast_addr}:{port}")
                    
                    # Send broadcast queries using CoD5 specific source port
                    # First getstatus to filter by gamename, then getinfo for details
                    responses = await self._send_cod5_dual_broadcast_query(
                        broadcast_addr, port
                    )
                    
                    # Process responses (already filtered and parsed)
                    self.logger.debug(f"Processing {len(responses)} validated CoD5 servers")
                    for server_info in responses:
                        servers.append(server_info)
                        self.logger.info(f"Discovered CoD5 server: {server_info.ip_address}:{server_info.port}")
            ...
            self.logger.info(f"CoD5 discovery complete: Found {len(servers)} servers")
            
        except Exception as e:
            self.logger.error(f"Error during CoD5 broadcast discovery: {e}")
        
        return servers
    
    async def _send_cod5_dual_broadcast_query(self, broadcast_addr: str, port: int) -> List[ServerResponse]:
        """
        Send CoD5 dual broadcast queries (getstatus + getinfo) and collect validated responses.
        ...
        Returns:
            List of ServerResponse objects for validated CoD5 servers
        """
        validated_servers = []
        all_responses = []
        ...
        try:
            ...
            self.logger.debug(f"Sent CoD5 getstatus query to {broadcast_addr}:{port}")
            ...
            for response_data, sender_addr in getstatus_responses:
                if self._is_valid_cod5_response(response_data):
                    try:
            ...
            if not valid_server_addresses:
                self.logger.debug("No valid CoD5 servers found after gamename filtering")
            ...
            self.logger.debug(f"Sent CoD5 getinfo query to {broadcast_addr}:{port}")
            ...
            for response_data, sender_addr in all_responses:
                if sender_addr in valid_server_addresses and self._is_valid_cod5_response(response_data):
                    try:
                        server_info = await self._parse_cod5_getinfo_response(response_data, sender_addr)
                        if server_info:
                            validated_servers.append(server_info)
                    except Exception as e:
                        self.logger.error(f"Error parsing CoD5 getinfo response from {sender_addr[0]}:{sender_addr[1]}: {e}")
    ...
    def _is_valid_cod5_response(self, data: bytes) -> bool:
        """
        Validate if response data is a valid CoD5 server response.
        
        Args:
            data: Response data to validate
            
        Returns:
            True if valid CoD5 response, False otherwise
        """
        if len(data) < 16:  # Minimum response size
            return False
        
        # Check for CoD5 response header (4 bytes of 0xFF)
    ...
    def _check_gamename_from_response(self, data: bytes) -> bool:
        """
        Quick check if response contains gamename="Call of Duty 5".
        
        Args:
            data: Raw response data from getstatus query
            
        Returns:
            True if gamename matches "Call of Duty 5", False otherwise
        """
        try:
            ...
            return gamename == 'Call of Duty: World at War'
            
        except Exception:
            return False
    
    async def _parse_cod5_getinfo_response(self, data: bytes, sender_addr: Tuple[str, int]) -> Optional[ServerResponse]:
        """
        Parse CoD5 server getinfo response data.
        ...
        """
        try:
            # Create a temporary CoD5 client to parse the response
            cod5_client = CoD5(sender_addr[0], sender_addr[1], self.timeout)
            ...
            # Parse key-value pairs (CoD5 uses backslash as delimiter)
            server_data = self._parse_key_value_pairs(kv_data)
            
            # Extract standard server information (from getinfo response)
            hostname = server_data.get('hostname', 'Unknown CoD5 Server')
            ...
            self.logger.error(f"Error parsing CoD5 response from {sender_addr[0]}:{sender_addr[1]}: {e}")
            return None
    
    def _parse_key_value_pairs(self, data: str) -> Dict[str, str]:
        """
        Parse CoD5 key-value pairs from response data.
