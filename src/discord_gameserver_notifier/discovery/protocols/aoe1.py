"""
Age of Empires 1 DirectPlay protocol implementation for game server discovery.
"""
...
class AoE1Protocol(ProtocolBase):
    ...
    def get_discord_fields(self, server_info: dict) -> list:
        """
        Get additional Discord embed fields for Age of Empires 1 servers.
        ...
        if 'game_mode' in server_info and server_info['game_mode']:
            mode_text = server_info['game_mode']
            if mode_text == 'Random Map':
                mode_text = '🗺️ Zufällige Karte'
            elif mode_text == 'Death Match':
                mode_text = '⚔️ Deathmatch'
            elif mode_text == 'Scenario':
                mode_text = '📜 Szenario'
        ...
        # Add DirectPlay specific info
        fields.append({
            'name': '🔧 Protokoll',
            'value': 'DirectPlay (AoE1)',
            'inline': True
        })
    ...
    async def scan_servers(self, scan_ranges: List[str]) -> List[ServerResponse]:
        """
        Scan for Age of Empires 1 servers using DirectPlay protocol.
        
        DirectPlay works by:
        1. Opening a TCP server on port 2300 to receive responses
        ...
        Returns:
            List of ServerResponse objects for AoE1 servers
        """
        servers = []
        udp_port = self.protocol_config['port']  # 47624
        
        self.logger.info("🎮 Starting AoE1 DirectPlay server discovery...")
        ...
        for network_range in scan_ranges:
            try:
                ...
                self.logger.info(f"📡 DirectPlay discovery for AoE1: {broadcast_addr}:{udp_port}")
                ...
                discovered_servers = await self._directplay_discovery(
                    broadcast_addr, udp_port, self.protocol_config['query_data']
                )
                
                self.logger.info(f"🔍 DirectPlay found {len(discovered_servers)} potential AoE1 servers")
                ...
                for server_ip, server_port in discovered_servers:
                    try:
                        ...
                        server_info_dict = await self._query_aoe1_server_via_opengsq(
                        ...
                        if server_info_dict:
                            ...
                            self.logger.info(f"✅ AoE1 server discovered: {server_ip}:{server_port}")
                            ...
                            self.logger.debug(f"❌ Could not get details for AoE1 server {server_ip}:{server_port}")
                        
                    except Exception as e:
                        self.logger.debug(f"❌ Failed to query AoE1 server {server_ip}:{server_port}: {e}")
        ...
        self.logger.info(f"🎯 AoE1 DirectPlay discovery completed: {len(servers)} servers found")
        return servers
    
    async def _directplay_discovery(self, broadcast_addr: str, udp_port: int, query_data: bytes) -> List[Tuple[str, int]]:
        """
        ...
        try:
            ...
            # Start TCP server on port 2300 (or next available port)
            tcp_server = None
            tcp_port = 2300
            for port_offset in range(10):  # Try ports 2300-2309
                try:
                    tcp_port = 2300 + port_offset
                    ...
                    if port_offset == 9:  # Last attempt
                        raise Exception(f"Could not bind TCP server to ports 2300-2309: {e}")
                    continue
            
            self.logger.info(f"🌐 DirectPlay TCP server listening on port {tcp_port}")
    ...
    async def _query_aoe1_server_via_opengsq(self, host: str, port: int) -> Optional[Dict[str, Any]]:
        """
        Query an AoE1 server using opengsq-python library to get complete server information.
        ...
        """
        try:
            self.logger.debug(f"Querying AoE1 server {host}:{port} via opengsq-python...")
            
            # Create AoE1 protocol instance
            aoe1_client = AoE1(host, port, self.timeout)
            
            # Get server info using opengsq-python
            server_info = await aoe1_client.get_status()
            
            # Convert Status object to dictionary format
            # server_info is a Status object, not a dict, so we access attributes directly
            info_dict = {
                'name': getattr(server_info, 'name', f'AoE1 Server {host}:{port}'),
                'map': getattr(server_info, 'map', 'Unknown Map'),
                'game': getattr(server_info, 'game_type', 'Age of Empires'),
                'players': getattr(server_info, 'num_players', 0),
                'max_players': getattr(server_info, 'max_players', 8),
                'game_version': getattr(server_info, 'game_version', '1.0c'),
                'game_mode': getattr(server_info, 'game_mode', 'Random Map'),
                'raw': getattr(server_info, 'raw', {})
            }
            
            self.logger.debug(f"✅ opengsq-python returned AoE1 server info for {host}:{port}")
            self.logger.debug(f"   Game version: {info_dict.get('game_version')}")
            self.logger.debug(f"   Mode: {info_dict.get('game_mode')}")
            self.logger.debug(f"   Map: {info_dict.get('map')}")
            
            return info_dict
            
        except Exception as e:
            self.logger.debug(f"❌ Error querying AoE1 server {host}:{port} via opengsq-python: {e}")
            return None
